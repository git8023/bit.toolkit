...
export class AsyncArrayStream<T, R = any> {
  ...
  next(): Promise<void> {
    return Functions.execOrAsyncGetter<any>(() => {
      ...
      return Functions
        ...
        .then((itemResult) => {
          if (itemResult === false) {
            console.log(`元素迭代返回结果:[${ itemResult }], 是否终止后续处理:[${ !itemResult }]`, itemData);
  ...
  private await(): Promise<void> {
    ...
    this.waitPromise = Promises.control(prm);
   ...
   * @private
   */
  private catch(cause: any) {
    // @ts-ignore
    const args = { self: this, error: new Error('BrokenError', { cause: cause }) };
